...
| 代码 | [Python 标准库课程工程](./代码/agent-harness-course/) | FakeProvider、AgentRunner、SQLite/JSONL 事件、RunState、显式完成契约、模型/工具硬上限、精确重复观察检测、示例策略、进程内 Effect 缓存及 29 项测试；代码变化须同步更新测试和相关文档。 |
...
课程工程不需要 API Key。现有测试证明主循环、SQLite 本地事件事务与 revision 冲突、完成契约、模型/工具上限、示例路径拒绝策略、**同进程** Effect 结果复用和状态投影；不证明跨进程副作用去重、真实沙箱或 RepoFix 修复成功率。
