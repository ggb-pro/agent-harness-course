...
读完后应能画出一个 Agent Run 的状态机，解释模型输出为何不能直接执行，区分事件、Checkpoint、Trace 和 Memory，指出 Tool 成功后进程崩溃的重复副作用风险，并设计能证明改动没有破坏其他任务的评测。建议每讲先口述结论，再按[设计说明的运行方法](./设计说明.md#代码怎么运行与阅读)运行课程工程或阅读对应源码；[面经](./面经.md)可用于自测。
...
**动手**：在[课程 `runner.py`](./代码/sonic/src/sonic_agent/core/runner.py) 找到“模型没有 ToolCall 就申请完成验收”的分支；说出无完成契约为什么返回 `unverified`，以及固定文本通过为何仍不代表 RepoFix 成功。
...
建议按以下顺序推进：先按[运行方法](./设计说明.md#代码怎么运行与阅读)跑 Demo 和测试；再读[sonic 的目标与当前能力](./设计说明.md)，用[面经](./面经.md)自测；接入真实工具前先补策略、对账、失败恢复和评测用例，不能直接用教学策略开放写入。
