...
> 更新于 2026-09-17。本文是**学习主线**：先用一条 RepoFix 任务讲清因果关系，再观察 Claude Code、DeepSeek Harness（下称 DSH）和 nanobot 的公开思路。[Harness 工程教学](#下篇工程专题与公开项目对照)提供专题与源码阅读；自己的项目目标和能力边界看[sonic 设计](./设计说明.md)，面试自测看[独立题库](./面经.md)。不要把设计目标当成课程代码已实现能力。

### 学习路线与最终能力

读完后应能画出一个 Agent Run 的状态机，解释模型输出为何不能直接执行，区分事件、Checkpoint、Trace 和 Memory，指出 Tool 成功后进程崩溃的重复副作用风险，并设计能证明改动没有破坏其他任务的评测。建议每讲先口述结论，再运行下面的最小代码并跳到 sonic 源码核对；完整基线命令见[导航的快速验证](./README.md#快速验证)，[面经](./面经.md)可用于自测。

上篇所有“动手”都给出可直接复制运行的 PowerShell 命令。先在仓库根目录准备 Python 3.10+ 环境：
...
之后每个代码块可单独运行；`@' ... '@ | python -` 会把中间的 Python 示例交给解释器，不需要另建 `.py` 文件。若你的 Python 命令不叫 `python`，用本机可用的 Python 3.10+ 可执行文件替换。本页默认动手示例只触及内存，不改仓库或调用真实模型；**设计示意**会明确标出 sonic 尚未实现，代码跳转只指向真实存在的实现或测试。
...
第二行的 `completed` **只证明文字完全匹配**，不证明文件已修改或测试通过。跳到 sonic 的[Runner 完成分支](./代码/sonic/src/sonic_agent/core/runner.py#L179-L191)、[固定文本契约](./代码/sonic/src/sonic_agent/core/completion.py#L25-L32)与[无契约回归测试](./代码/sonic/tests/test_core_contract.py#L24-L39)核对。
...
当前会看到 `run.started`、模型请求/响应、验收与 `run.unverified`；设计稿里的 `MODEL_PENDING`、`WAITING_APPROVAL`、`NEEDS_RECONCILIATION` 等细阶段**尚未实现**。对照[运行循环](./代码/sonic/src/sonic_agent/core/runner.py#L74-L220)、[现有终态与投影](./代码/sonic/src/sonic_agent/core/state.py#L10-L29)和[循环测试](./代码/sonic/tests/test_runner_projection.py)，再用虚线补画目标状态，不要把虚线画成已有代码。
...
前两项输出 `False`，后两项输出 `True`。`True` 只表示**字面路径段未触发此过滤器**，不表示真实路径安全：`src/link` 若是指向仓库外的符号链接、`src/../outside.txt` 若越过允许目录，仓库能力仍必须拒绝。审批后改参也需要重新审批，但 sonic **尚无审批或沙箱实现**。跳到[教学策略](./代码/sonic/src/sonic_agent/capabilities/tools.py)、[路径策略测试](./代码/sonic/tests/test_tools.py)和[真实路径边界测试](./代码/sonic/tests/test_repository_tools.py)核对。

**动手 3B · 用 sonic 的真正只读边界读取自己的仓库。** 根目录由应用在创建能力时传入，Tool 参数没有 `root` 或 `base_dir`，所以模型不能在一次调用里把范围换到别处：

```powershell
@'
from pathlib import Path
from sonic_agent import ReadOnlyRepositoryTools, ToolRegistry, WorkspaceScope

scope = WorkspaceScope.open(Path.cwd(), allowed_paths=("src", "tests"))
tools = ReadOnlyRepositoryTools(scope)
registry = ToolRegistry()
tools.register(registry)

print(registry.names)
print(registry.execute("repo.list", {"path": "src", "depth": 2}).value)
print(registry.execute("repo.read", {"path": "src/sonic_agent/__init__.py",
                                     "start_line": 1, "max_lines": 30}).value)
print(registry.execute("repo.search", {"path": "src", "query": "ToolRuntime"}).value)
'@ | python -
```

从 `代码/sonic` 目录运行。返回中的 `trust=untrusted_repository_content` 提醒模型：代码是资料，不是更高优先级指令；读取带 SHA-256 与行范围，搜索是**字面匹配**而非正则。尝试把路径改成 `../.env`、绝对路径或符号链接会得到结构化拒绝；文件过大、二进制、非 UTF-8、过长行和扫描超限也有明确结果。实现跳到[repository.py](./代码/sonic/src/sonic_agent/capabilities/repository.py)，FakeProvider 的完整“列举 → 读取 → 搜索 → 回答”示例跳到[测试](./代码/sonic/tests/test_repository_tools.py)。这仍未冻结 Git SHA、发现/执行测试，也不抵御拥有同等宿主权限的恶意进程。
...
`1` 只说明记录过一次调用，**不能断言工具已执行、可以续跑或可以重试**。跳到[事件模型](./代码/sonic/src/sonic_agent/core/events.py#L15-L29)、[只读 Projection](./代码/sonic/src/sonic_agent/core/projections.py#L21-L48)、[RunState 投影](./代码/sonic/src/sonic_agent/core/state.py#L29-L82)及[Runner 拒绝已有 Run](./代码/sonic/src/sonic_agent/core/runner.py#L88-L89)核对。
...
建议按证据逐级推进，而非一口气实现完整平台。阶段 A：运行当前全部自动化测试，读懂 FakeProvider、Runner、Event Store、ToolRuntime、只读 Repository Tools，以及 OpenAI Responses 纯文本 Provider 的出站授权、环境变量凭据、审计元数据和安全错误边界；FakeProvider 驱动的只读 Tool Loop 已有测试，真实联网 smoke 与 OpenAI Function Calling 工具闭环仍待验证。阶段 B：已有可重建 RunState、SQLite 事件 revision、模型/工具次数与截止时间上限、显式 CompletionContract 和精确重复观察检测；仍需冻结 Git 版本、发现测试、接入 Context、token/费用预算、更稳健的进展检测及 RepoFix Verifier。阶段 C：持久化 Effect 意图、故障注入和对账；证明重启后不盲重试。阶段 D：在隔离 worktree 与真正的沙箱中实现 RepoFix，并建立固定任务集与简单基线。阶段 E：只有单 Agent 数据显示收益时才引入 Explore Subagent、MCP 或远程执行。sonic 的门禁以[演进顺序](./设计说明.md#演进顺序与依赖门禁)为准。

**动手 9 · 禁止远程时，会不会先偷读 Key 或创建客户端？** 下面直接调用 sonic 的真实适配器，但不授权远程；它会在导入 SDK、读取 `OPENAI_API_KEY` 或建立网络客户端之前失败关闭，因此无需安装 SDK 或设置 Key：

```powershell
@'
from sonic_agent import OpenAIResponsesProvider, ProviderError

provider = OpenAIResponsesProvider("model-chosen-by-user")
try:
    provider.complete([{"role": "user", "content": "hello"}])
except ProviderError as error:
    print(error.code)          # egress_denied
    print(error.safe_message)  # 可安全写入事件，不含 SDK 原始错误或凭据
'@ | python -
```

实现见 [OpenAIResponsesProvider](./代码/sonic/src/sonic_agent/models/openai_responses.py)，门禁顺序、严格布尔授权、每 Run 不复用授权、端点精确匹配、`store=False`、禁用 SDK 隐式重试/HTTP 重定向/环境代理、usage/时延映射和异常链不带密钥的证据见 [Provider 边界测试](./代码/sonic/tests/test_openai_provider.py)。只有官方端点会默认读取 `OPENAI_API_KEY`；自定义远程端点必须显式选择另一个 `credential_env`，防止把默认凭据送错主机。纯文本基线会直接拒绝 `function_call` 和工具历史；这证明的是确定性边界，不是远程服务已经调用成功。会发送数据并可能产生费用的手工 smoke 单独放在[导航](./README.md#快速验证)，`store=False` 也不等于 Zero Data Retention。
...
这六周是**后续实现路线，不是 sonic 当前交付进度**。现有教学代码先做上篇的[动手 1](#第一讲harness-究竟解决了什么)、[动手 3/3B](#第三讲模型建议到-tool-执行之间必须过哪些门)、[动手 4](#第四讲事件日志为什么不等于普通日志)、[动手 5](#第五讲最难的失败窗口副作用已发生结果未记录)、[动手 6](#第六讲context-不只是把历史塞进窗口)与[动手 9](#第十讲从课程原型走到作品集项目)。只读 Repository Tools 与 OpenAI Responses 纯文本适配器已有代码和离线边界测试，但尚未完成 Git 版本冻结、测试发现、真实联网 smoke；OpenAI Function Calling、审批、沙箱、跨进程恢复、多 Agent、插件和完整评测仍待实现验证。不要把下面的目标列表当成已有代码。
...
sonic 当前的纯文本基线见 [OpenAIResponsesProvider](./代码/sonic/src/sonic_agent/models/openai_responses.py) 与 [Provider 边界测试](./代码/sonic/tests/test_openai_provider.py)；这些测试 mock OpenAI SDK 构造器和返回值，不代表真实网络或工具闭环。生产构造器没有客户端、传输工厂或明文 Key 注入入口。
...
不要把 Golden Trace 绑定到时间戳或随机标识。先规范化非确定字段，再比较事件类型、关键载荷和因果顺序。配套代码以 `FakeProvider` 保持主循环测试确定性，并通过 mock SDK 构造器测试 OpenAI Responses 适配边界；自动化测试无需真实 API Key 或联网。SQLite revision、虚假完成、取消、预算、精确重复观察、每 Run 远程授权和错误脱敏样本体现这条路线，完整 Golden Trace 与真实模型评测仍待实现。
...
采用路径：**观察 → 固定问题与基线 → 小实验 → 评测失败/收益 → 设计说明内的 ADR 式决策记录 → 代码、测试、能力状态和文档**。如果只有“看起来先进”而无第二个真实任务或失败样例，留在观察队列，不改内核。季度复盘时删除失效链接、纠正过时描述，而不是静默覆盖旧结论。
