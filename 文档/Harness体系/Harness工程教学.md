...
> 配套内容：[sonic 设计](../sonic设计/README.md)、[独立面经](../../面经/README.md)与[`代码/sonic`](../../代码/sonic/README.md)。
...
本节保留原课程的练习样例与目录草图，**不是 sonic 当前产品目录或已交付功能清单**。sonic 的正式目标、架构和门禁以[设计文档](../sonic设计/README.md)为准；练习中的容器、Subagent、Trace 页面等当前尚未实现。
...
这套机制来源于经典的 Outbox、幂等键和对账思想，是 sonic 自己采用的可靠性设计，不应错误归因于某一个 Agent 项目。
...
建议按以下顺序推进：先运行 [`代码/sonic`](../../代码/sonic/README.md) 的 Demo 和测试；再读[sonic 的目标与当前能力](../sonic设计/README.md)，用[面经题库](../../面经/题库/Agent-Harness-48题.md)自测；接入真实工具前先补策略、对账、失败恢复和评测用例，不能直接用教学策略开放写入。
