...
> 更新于 2026-09-15。本文是**学习主线**：先用一条 RepoFix 任务讲清因果关系，再观察 Claude Code、DeepSeek Harness（下称 DSH）和 nanobot 的公开思路。[Harness 工程教学](./Harness工程教学.md)提供专题与源码阅读；自己的项目目标和能力边界看[sonic 设计](../sonic设计/README.md)，面试自测看[独立题库](../../面经/题库/Agent-Harness-48题.md)。不要把设计目标当成课程代码已实现能力。

## 学习路线与最终能力

读完后应能画出一个 Agent Run 的状态机，解释模型输出为何不能直接执行，区分事件、Checkpoint、Trace 和 Memory，指出 Tool 成功后进程崩溃的重复副作用风险，并设计能证明改动没有破坏其他任务的评测。建议每讲先口述结论，再运行[课程工程](../../代码/sonic/README.md)或阅读对应源码；题库里相关追问可用于自测。
...
**动手**：在[课程 `runner.py`](../../代码/sonic/src/sonic_agent/core/runner.py) 找到“模型没有 ToolCall 就完成”的分支；说出它为何只能表示模型停止，而不是 RepoFix 任务成功。
...
**动手**：用 [Projection](../../代码/sonic/src/sonic_agent/core/projections.py) 从事件重建工具次数；再思考“重建统计”与“安全恢复未完成 Tool”为何不是同一件事。
